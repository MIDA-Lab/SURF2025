...
class OPT_attack_lf(object):
    ...
    def attack_untargeted(self, x0, y0, alpha = 0.2, beta = 0.01, iterations = 1000):
        ...
        # Convert x0 to numpy array if it's on GPU
        ...
        for i in range(num_directions):
            query_count += 1
            theta = np.random.randn(*x0_np.shape)  # Use x0_np.shape instead of x0.shape
            # Convert theta to tensor and move to correct device
            theta_tensor = torch.from_numpy(theta).float().to(x0.device)
            if model.predict_label(x0 + theta_tensor) != y0:
        ...
        timeend = time.time()
        if g_theta == np.inf:
            return None, float('inf'), 0  # Return None instead of "NA"
        ...
        # Convert best_theta to tensor before final predictions
        best_theta_tensor = torch.from_numpy(best_theta).float().to(x0.device)
        target = model.predict_label(x0 + g_theta * best_theta_tensor)
        timeend = time.time()
        print("\nAdversarial Example Found Successfully: distortion %.4f target %d queries %d \nTime: %.4f seconds" % (g_theta, target, query_count + opt_count, timeend-timestart))
        #return x0 + g_theta*best_theta, g_theta, query_count + opt_count
        # Return the adversarial example
        adv = x0 + g_theta * best_theta_tensor
        return adv, g_theta, query_count + opt_count

    def predict_with_theta(self, x0, theta):
        """Helper function to handle device conversion"""
        theta_tensor = torch.from_numpy(theta).float().to(x0.device)
        return self.model.predict_label(x0 + theta_tensor)

    def fine_grained_binary_search_local(self, model, x0, y0, theta, initial_lbd = 1.0, tol=1e-5):
        nquery = 0
        lbd = initial_lbd
        theta_tensor = torch.from_numpy(theta).float().to(x0.device)
        if model.predict_label(x0 + lbd * theta_tensor) == y0:
            lbd_lo = lbd
            lbd_hi = lbd*1.01
            nquery += 1
            while model.predict_label(x0+lbd_hi*theta_tensor) == y0:
                lbd_hi = lbd_hi*1.01
                nquery += 1
                if lbd_hi > 20:
            ...
            lbd_lo = lbd*0.99
            nquery += 1
            while model.predict_label(x0+lbd_lo*theta_tensor) != y0 :
                lbd_lo = lbd_lo*0.99
                nquery += 1
        ...
        while (lbd_hi - lbd_lo) > tol:
            ...
            nquery += 1
            if model.predict_label(x0 + lbd_mid*theta_tensor) != y0:
                lbd_hi = lbd_mid
            else:
                lbd_lo = lbd_mid
    ...
    def fine_grained_binary_search(self, model, x0, y0, theta, initial_lbd, current_best):
        ...
        if initial_lbd > current_best: 
            if model.predict_label(x0+current_best*theta_tensor) == y0:
                nquery += 1
                return float('inf'), nquery
            lbd = current_best
        else:
    ...
    def attack_targeted(self, initial_xi, x0, y0, target, alpha = 0.2, beta = 0.001, iterations = 5000):
        ...
        if (model.predict_label(x0) != y0):
            print("Fail to classify the image. No need to attack.")
            return None, 0, 0  # Return None instead of x0
    ...
    def __call__(self, input_xi, label_or_target, initial_xi=None, target=None, TARGETED=False):
        if TARGETED:
            adv, g_theta, qc = self.attack_targeted(initial_xi, input_xi, label_or_target, target)
        else:
            adv, g_theta, qc = self.attack_untargeted(input_xi, label_or_target)
        return adv, g_theta, qc
